                ...
                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8">
                    ...
                    <tr v-if="showDetails[index]" class="details-row">
                        <td colspan="100%" class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                            <div v-if="item.questions.length > 0" class="p-4 bg-gray-50 rounded-lg shadow-md ">
                                <!-- Carusel Întrebări -->
                                <Carousel :items-to-show="1">
                                    <Slide v-for="(question, qIndex) in item.questions" :key="qIndex">
                                        <!-- Detalii întrebare -->
                                        <div class="p-4 bg-white rounded shadow-md w-3/6">

                                            <h3 class="text-lg font-semibold text-gray-700">
                                                Întrebarea {{ qIndex + 1 }}: {{ question.question }}
                                            </h3>

                                            <p class="text-sm text-gray-600 mb-2">Scor: {{
                                                question.score }}</p>

                                            <!-- Răspunsuri -->
                                            <button @click="toggleAnswers(index, qIndex)"
                                                class="text-blue-500 underline mb-2">
                                                {{ showAnswers[index][qIndex] ? 'Ascunde' : 'Afișează'
                                                }} Răspunsuri
                                            </button>

                                            <ul v-if="showAnswers[index][qIndex]" class="space-y-2 mt-2 ">
...
<GenericDeleteNotification :open="isDeleteDialogOpen" @update:open="isDeleteDialogOpen = $event" title="Delete Item"
    :message="messageToDelete" :deleteRoute="routeToDelete" :objectId="itemToDelete" />
...
export default {
    ...
    methods: {
        ...
        toggleAnswers(itemIndex, questionIndex) {
            ...
            this.showAnswers[itemIndex][questionIndex] = !this.showAnswers[itemIndex][questionIndex];
        ...
        toggleDetails(index) {
            ...
            if (!this.showDetails[index]) {
                this.showAnswers[index] = new Array(this.items.data[index].questions.length).fill(false);
            }
        },
        ...
        deleteAnswer(answer) {
            ...
            this.routeToDelete = 'admin.answers.destroy';
            this.messageToDelete = "Are you sure you want to delete this answer?";
        }
        ,
        deleteQuestion(question) {
            this.isDeleteDialogOpen = !this.isDeleteDialogOpen;
            this.itemToDelete = question.id;
            this.routeToDelete = 'admin.questions.destroy';
            this.messageToDelete = "Are you sure you want to delete this question?";
        },
        deleteQuiz(quiz) {
            this.isDeleteDialogOpen = !this.isDeleteDialogOpen;
            this.itemToDelete = quiz.id;
            this.routeToDelete = 'admin.user_quizzes.destroy';
