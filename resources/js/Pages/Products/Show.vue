        ...
        <main class="mt-32 mb-20">
            <div class="bg-white">
                <div class="mx-auto max-w-7xl overflow-hidden sm:px-6 lg:px-8 ">
                    ...
                    <div class="lg:grid lg:grid-cols-7 lg:gap-x-8 lg:gap-y-10 xl:gap-x-16 p-6">
                        <!-- Product Image -->
                        <div class="lg:col-span-4 lg:row-end-1 relative">
                            <div
                                ...
                                <img :src="product.image" alt="imageAlt" class="max-h-full max-w-full object-cover" />
                            </div>
                            <div
                        ...
                        <div
                            ...
                            <div class="mt-4">
                                ...
                                <h2 class="text-xl font-semibold text-gray-700 sm:text-2xl mt-2">📌 Category: {{
                                    product.category }}</h2>
                            ...
                            <p class="mt-6 text-gray-700 text-lg leading-relaxed">🌟 {{ product.description }}</p>
                            <div class="mt-6 text-2xl font-semibold">
                                ...
                                <span :class="{ 'text-green-600': product.old_price }">💸 {{ product.price }} RON</span>
                            </div>

                            <!-- Quantity Selector -->
                            <div class="mt-6">
                                <label for="quantity" class="block text-lg font-medium text-gray-800">🔢 Choose
                                    Quantity</label>
                                <select id="quantity" v-model="quantity"
                                    ...
                                    <option v-for="num in 10" :key="num" :value="num">🍽️ {{ num }}</option>
                                </select>
                            </div>
                        ...
                        <div class="mt-10 p-6 rounded-lg shadow-md bg-gray-50 w-full lg:col-span-7">
                            <h2 class="text-2xl font-bold text-center text-gray-900 mb-4">🍽️ Crunchy Nutritional
                                Breakdown</h2>
                            <div class="overflow-hidden rounded-lg">
                                <table class="min-w-full bg-white rounded-lg shadow-md text-gray-700">
                                    <tbody>
                                        <tr class="border-b px-6 py-4 flex justify-between">
                                            <td class="flex items-center">🚀 Super Energy (Calories)</td>
                                            <td class="font-semibold">{{ product.calories }} kcal</td>
                                        </tr>
                                        <tr class="border-b px-6 py-4 bg-gray-100 flex justify-between">
                                            <td class="flex items-center">💪 Muscle Booster (Protein)</td>
                                            <td class="font-semibold">{{ product.protein }} g</td>
                                        </tr>
                                        <tr class="border-b px-6 py-4 flex justify-between">
                                            <td class="flex items-center">🍞 Energy Fuel (Carbs)</td>
                                            <td class="font-semibold">{{ product.carbs }} g</td>
                                        </tr>
                                        <tr class="border-b px-6 py-4 bg-gray-100 flex justify-between">
                                            <td class="flex items-center">🥑 Healthy Fats</td>
                                            <td class="font-semibold">{{ product.fats }} g</td>
                                        </tr>
                                        <tr class="border-b px-6 py-4 flex justify-between">
                                            <td class="flex items-center">🌿 Digestion Hero (Fiber)</td>
                                            <td class="font-semibold">{{ product.fiber }} g</td>
                                        </tr>
                                        <tr class="border-b px-6 py-4 bg-gray-100 flex justify-between">
                                            <td class="flex items-center">🍭 Sweetness Level (Sugar)</td>
                                            <td class="font-semibold">{{ product.sugar }} g</td>
                                        </tr>
                                        <tr class="border-b px-6 py-4 flex justify-between">
                                            <td class="flex items-center">🧑‍🍳 Ingredients</td>
                                            <td class="font-semibold">{{ product.ingredients.join(', ') }}</td>
                                        </tr>
                                        <tr class="px-6 py-4 bg-gray-100 flex justify-between">
                                            <td class="flex items-center">⚠️ Allergens</td>
                                            <td class="font-semibold">{{ product.allergens.join(', ') }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
