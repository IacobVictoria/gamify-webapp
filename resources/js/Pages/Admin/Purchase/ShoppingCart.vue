    ...
    <div class="bg-white">
        <div class="mx-auto max-w-4xl px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
            ...
            <form class="mt-12">
                            ...
                            <div class="relative ml-4 flex flex-1 flex-col justify-between sm:ml-6">
                                <div>
                                    <div class="flex justify-between sm:grid sm:grid-cols-2">
                                        ...
                                        <p class="text-right text-sm font-medium text-gray-900">{{ item.product.price }}
                                        </p>
                                        <div>
                                            <label for="quantity-{{ item.product.id }}"
                                                class="block text-sm font-medium text-gray-700">Cantitate:</label>
                                            <select id="quantity-{{ item.product.id }}"
                ...
                <div class="mt-10 sm:ml-32 sm:pl-6">
                        ...
                        <div class="flow-root">
                            <dl class="-my-4 divide-y divide-gray-200 text-sm">
                                ...
                                <div class="flex items-center justify-between py-4">
                                    ...
                                    <dd class="text-base font-medium text-gray-900">{{ calculateTotal().toFixed(2) }}
                                    </dd>
                                </div>
                            </dl>
                        </div>
                    ...
                    <div class="mt-6 text-center text-sm text-gray-500">
                        <p>
                            or{{ ' ' }}
                            <a :href="route('admin.suppliers_products.index')"
            ...
            <GenericDeleteNotification :open="isDeleteDialogOpen" @update:open="isDeleteDialogOpen = $event"
                ...
                :deleteRoute="'admin.shopping-cart.destroy'" :objectId="itemToDelete" />

            <GenericDeleteNotification :open="isDeleteCheckoutDialogOpen"
                @update:open="isDeleteCheckoutDialogOpen = $event" title="Out of Stock Items"
                message="There are items out of stock in your cart! Do you want to remove all out-of-stock items?"
                :items="outOfStockItems" :deleteRoute="'admin.shopping-cart.destroy'" />
...
export default {
    name: 'Admin/Purchase/ShoppingCart',
    components: {
    ...
    methods: {
        ...
        updateQuantity(productId, newQuantity) {
            ...
            this.$inertia.post(route('admin.shopping-cart.update', productId), {
        ...
        handleCheckout(event) {
            ...
            if (!allItemsInStock) {
                ...
                this.$inertia.visit(route('admin.checkout.index'));
            }
        }
    },
