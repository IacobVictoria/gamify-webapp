...
<template>
    ...
    <div class="py-8">
      <div class="px-32">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="mb-10 w-full bg-[#334155] text-white p-10  text-left">
            <span class="text-xl block text-left ml-0">Hello, {{ $page.props.user.name }} 👋</span>
          </div>
        
         <Stats :weeklySales="weeklySales" :weeklyOrders="weeklyOrders" :newVisitors="newVisitors" :salesChange="salesChange" :ordersChange="ordersChange" :newVisitorsChange="newVisitorsChange" ></Stats>
         <TopSellingProducts :topProductsWeekly="topProductsWeekly"/>
         <CurrentDiscountsProgress :currentDiscounts="currentDiscounts"/>
  ...
  <GlobalStats :globalStats="globalStats" class="w-full md:w-1/2" />

  <!-- NPS & Chart ocupa 50% -->
  <div class="w-full md:w-1/2 flex flex-col gap-5">
    <div class="bg-white p-4 ">
      <h1 class="title text-left text-xl font-semibold">{{ nps !== null ? nps : 'N/A' }}</h1>
      <p class="text-gray-500">Net Promoter Score (NPS)</p>
    </div>
