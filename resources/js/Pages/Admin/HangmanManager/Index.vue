        ...
        <div class="px-24 py-6">
            <div class="w-full mx-auto sm:px-6 lg:px-8">
                <div class="px-6 py-12">
                    <h1 class="text-3xl font-bold text-center mb-10 text-gray-700">🎮 Hangman Game WORDS Manager</h1>

                    <div class="flex gap-6 p-6 ">
                        <!-- Left Panel - Word List -->
                        <div class="w-1/2 bg-white p-4 rounded-lg shadow-lg max-h-[600px] overflow-y-auto">
                            <h2 class="text-xl font-semibold mb-4">Lista de Cuvinte</h2>
                            <ul>
                                <li v-for="wordObj in words" :key="wordObj.word" class="border-b py-2">
                                    <div class="flex justify-between items-center">
                                        <span class="font-bold">{{ wordObj.word }}</span>
                                        <button @click="toggleHint(wordObj.word)" class="text-blue-500">
                                            {{ expandedWord === wordObj.word ? "▲" : "▼" }}
                                        </button>
                                    </div>
                                    <div v-if="expandedWord === wordObj.word" class="mt-2 text-gray-600">
                                        <p>{{ wordObj.hint }}</p>
                                        <div class="flex gap-2 mt-2">
                                            <button @click="startEdit(wordObj)" class="text-yellow-500">Edit</button>
                                            <button @click="deleteWord(wordObj.word)"
                        ...
                        <div
                            ...
                            <div class="max-w-md w-full bg-white p-6 rounded-xl shadow-lg">
                                <h2
                                    ...
                                    <span>✏️</span>
                                    {{ editMode ? "Editează Cuvântul" : "Adaugă un Cuvânt Nou" }}
                                </h2>
                                ...
                                <div class="flex justify-center gap-4">
                                    <button v-if="editMode" @click="saveEdit"
                                    ...
                                    <button v-if="!editMode" @click="addWord"
...
export default {
    ...
    computed: {
        wordInput: {
            get() {
                return this.editMode && this.editWord ? this.editWord.word : this.newWord;
            },
            set(val) {
        ...
        hintInput: {
            get() {
                return this.editMode && this.editWord ? this.editWord.hint : this.newHint;
            },
            set(val) {
                if (this.editMode && this.editWord) {
