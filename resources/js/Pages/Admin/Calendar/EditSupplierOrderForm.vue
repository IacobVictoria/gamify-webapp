<template>
    <div>
        <h3 class="text-base font-semibold text-gray-900">Edit Command</h3>
...
export default {
    ...
    data() {
        const details = this.event?.details ? JSON.parse(this.event.details) : {};
        return {
            formData: useForm({
                id: this.event?.id ?? '',
        ...
        'formData.supplierId': {
            handler(newSupplierId) {
                if (newSupplierId && this.products[newSupplierId]) {
                    ...
                    this.selectedProductIds = Object.keys(this.formData.productQuantities);
                } else {
        ...
        selectedProductIds: {
            handler(newSelected) {
                ...
                newSelected.forEach(productId => {
                    newQuantities[productId] = this.formData.productQuantities[productId] || 1;
                });
    ...
    methods: {
        getMaxQuantity(productId) {
            const product = this.selectedProducts.find(product => product.id === productId);
        ...
        submitForm() {
            ...
            const formattedProductQuantities = this.selectedProductIds.map(productId => {
                const product = this.selectedProducts.find(p => p.id === productId);
                return {
                    productId,
                    productName: product ? product.name : '',
                    quantity: this.formData.productQuantities[productId] || 1
                };
            });

            this.formData.details = JSON.stringify({
                supplier: this.formData.supplierId,
                supplierName: this.suppliers.find(s => s.id === this.formData.supplierId)?.name ?? '',
                productQuantities: formattedProductQuantities
            });

            console.log("Submitting formData:", this.formData);

            this.$inertia.put(route('admin.calendar.event.update', { id: this.formData.id }), {
