    ...
    <AuthenticatedLayout>
        <div class="m-12 pl-32 pr-32 grid grid-cols-3 gap-6">
            ...
            <div class="col-span-3 text-center mb-4">
                <h1 class="text-2xl font-bold text-blue-700 flex items-center justify-center">
                    ✍️ Let's Create an Awesome Quiz!
                </h1>
            </div>
            <!-- Tips & Tricks (Coloana dreaptă) -->
            <div class="bg-yellow-50 p-6 rounded-md shadow-md border border-yellow-200 flex flex-col justify-center">
                <h2 class="font-bold text-yellow-700 flex items-center text-lg mb-4">
                    💡 Quick Tips for a Great Quiz!
                </h2>
                <ul class="list-disc list-inside text-base text-gray-800 space-y-3">
                    <li>🧐 <span class="font-semibold">Make sure your questions are clear and concise.</span></li>
                    <li>🔢 <span class="font-semibold">Provide at least four answer choices.</span></li>
                    <li>🎯 <span class="font-semibold">Balance between easy and difficult questions.</span></li>
                    <li>💡 <span class="font-semibold">Add some fun & creative questions.</span></li>
                    <li>🏆 <span class="font-semibold">Assign logical points for each question.</span></li>
                </ul>
            </div>

            <!-- Formularul de creare a quiz-ului (Coloana stângă) -->
            <div class="col-span-2 p-12 bg-white shadow-md rounded-lg border border-gray-200">
                <h2 class="text-lg font-semibold mb-4">📋 Quiz Details</h2>

                <div class="mb-3">
                    <label class="block text-gray-600 font-medium text-sm mb-1">Quiz Title</label>
                    <input v-model="quizTitle" placeholder="Give your quiz an engaging title"
                        class="w-full p-2 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-400" />
                    <p v-if="errors.title" class="text-red-500 text-xs mt-1">⚠️ {{ errors.title }}</p>
                </div>

                <div class="mb-3">
                    <label class="block text-gray-600 font-medium text-sm mb-1">Quiz Description</label>
                    <textarea v-model="quizDescription" placeholder="Describe what this quiz is about..."
                        class="w-full p-2 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-400"></textarea>
                    <p v-if="errors.description" class="text-red-500 text-xs mt-1">⚠️ {{ errors.description }}</p>
                </div>
                <div class="mb-3">
                    ...
                    <p v-if="errors.difficulty" class="text-red-500 text-xs mt-1">⚠️ {{ errors.difficulty }}</p>
                </div>
                ...
                <!-- Lista întrebărilor -->
                <div class="mt-6">
                    <h5 class="text-md font-semibold mb-2">📝 Your Questions</h5>
                    <div v-for="(question, index) in questions" :key="index"
                        class="p-3 border border-gray-200 bg-gray-50 rounded-md mb-2 text-sm">
                        <h5 class="font-semibold text-gray-800">{{ question.text }}
                            <span class="text-gray-500">(🎯 Score: {{ question.score }})</span>
                        </h5>
                        <ul class="ml-4 text-gray-600">
                            <li v-for="(answer, ansIndex) in question.answers" :key="ansIndex">
                                ✅ {{ ansIndex + 1 }}. {{ answer.text }}
                                <span v-if="answer.isCorrect" class="text-green-500 font-bold">(Correct)</span>
                            </li>
                        </ul>
                    </div>
                    <p v-if="errors.questions" class="text-red-500 text-xs mt-1">⚠️ {{ errors.questions }}</p>
                </div>

                <!-- Final Save Button -->
                <div class="mt-6 flex justify-center">
                    <button @click="postQuiz"
        ...
        <div v-if="isQuestionModalOpen"
            ...
            <div class="bg-white p-5 rounded-lg shadow-lg w-full max-w-lg mx-auto border border-gray-200">
                <h2 class="text-xl font-bold text-center mb-3 text-blue-500">✏️ Add a New Question</h2>

                <div class="mb-3">
                    <label class="block text-gray-700 font-medium text-sm mb-1">Question</label>
                    <input v-model="newQuestionText" placeholder="Enter your question here..."
                        class="w-full p-2 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-400" />
                </div>

                <div class="mb-3">
                    <label class="block text-gray-700 font-medium text-sm mb-1">Score</label>
                    <input v-model="newQuestionScore" type="number" placeholder="Set a score"
                        class="w-full p-2 text-sm border border-gray-300 rounded focus:ring-2 focus:ring-blue-400" />
                </div>

                <h5 class="text-gray-700 font-semibold mb-2">📝 Possible Answers</h5>
                <div class="grid grid-cols-3 mb-2 text-sm text-gray-600 font-semibold">
                ...
                <div v-for="(answer, index) in newAnswers" :key="index" class="grid grid-cols-3 items-center mb-2">
                    <div class="flex items-center col-span-2">
                        ...
                        <button @click="removeAnswer(index)" class="text-red-500 ml-2 hover:text-red-700 text-sm">
                            ❌
                        </button>
                    </div>
