<template>
    <div class="notification-icon" @click="toggleDropdown()">
      ...
      <div v-if="isDropdownVisible" class="dropdown">
        <Dropdown :notifications="notifications" :handle="false" />
      </div>
    </div>
  </template>
    ...
    methods: {
      async toggleDropdown() {
        ...
        if (this.isDropdownVisible && this.unreadNotifications > 0) {
          await axios.post(route('admin-gamification.notifications.markAsRead'));
          this.unreadNotifications = 0;
        }
      },
      async fetchUnreadNotifications() {
        try {
          const response = await axios.get(route('admin-gamification.notifications.getNotifications'));
