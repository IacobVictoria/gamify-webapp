    ...
    <div class="container mx-auto mt-5 w-full md:w-1/2">
        <div class="card bg-white shadow-md rounded-lg p-5">
            ...
            <div class="card-body mt-4">
                <div
                    v-if="successMessage"
                    class="alert alert-success bg-green-200 text-green-700 p-3 rounded"
                >
                    {{ successMessage }}
                </div>
                ...
                <div
                    ref="cardElement"
                    class="mt-4 border p-3 bg-white rounded"
                ></div>

                <button
                    @click="handleSubmit"
                    class="w-full mt-4 bg-blue-600 text-white p-2 rounded"
                    :disabled="processing"
                >
                    Pay Now
                </button>
        ...
        async handleSubmit() {
            ...
            try {
                ...
                if (error) {
                    ...
                    await axios.post("/cancel-payment", {
                        order_id: this.order.id,
                    });
                } else {
                    this.successMessage = "Plată efectuată cu succes!";
                    try {
                        await axios.post("/confirm-payment", {
                            order_id: this.order.id,
                        });
                        setTimeout(() => {
                            this.$inertia.visit("/user/dashboard");
                        }, 2000);
                    } catch (error) {
                ...
                this.errorMessage = "Ceva nu a mers bine!";
            }
