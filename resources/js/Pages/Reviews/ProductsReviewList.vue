        ...
        <div class="-mb-10">
            ...
            <div v-if="reviews.length > 0">
                <div v-for="(review, reviewIdx) in reviews" :key="review.id" class="text-sm text-gray-500">
                    <div :class="[reviewIdx === 0 ? '' : 'border-t border-gray-300', 'py-3']">
                        <div class="flex items-start gap-3 p-2 rounded-lg shadow-md bg-white border border-gray-300">
                            <inertia-link class="flex flex-col items-center text-center no-underline"
                                :href="route('user.profile.show', { userId: review.user.id })">
                                <img :src="review.user.gender === 'Male' ? '/images/male.png' : '/images/female.png'"
                                    alt="User Avatar"
                                    class="w-14 h-14 rounded-full border-2 border-indigo-500 shadow-md" />

                                <div class="flex items-center text-base mt-1">
                                    <i v-if="review.isVerified" class="fa fa-shield-check text-green-500 text-xl"></i>
                                    <span class="text-gray-800 ml-2 font-semibold">
                                        {{ review.isVerified ? '✔️ Verificat' : '❌ Neconfirmat' }}
                                    </span>
                                </div>

                                <span class="font-semibold text-gray-900 text-base mt-2">{{ review.user.name }}</span>
                            ...
                            <div class="flex-1 items-center">
                                <div class="flex-1">
                                    <div class="flex flex-col items-center">
                                        <star-rating :star-size="22" :rating="review.rating" :read-only="true"
                                            :increment="0.5" :show-rating="false" class="mt-2" />
                                        <p class="text-gray-900 text-base mt-1 font-bold">⭐ {{ review.rating }} / 5</p>
                                    </div>
                                </div>
                                <!-- Titlu și Descriere -->
                                <div class="mt-2 text-base">
                                    <p class="font-semibold text-gray-900 flex items-center text-xl">
                                        <i class="fa fa-quote-left text-indigo-500 mr-2 text-xl"></i> {{
                                            review.title }}
                                    </p>
                                    <p class="text-gray-800 text-base mt-1">📝 {{ review.description }}</p>
                                </div>

                                <!-- Media Attachments -->
                                <div v-if="review.reviewMedia.length" class="mt-2">
                                    <h3 class="text-base font-semibold text-gray-900">📷 Media:</h3>
                                    <div class="flex gap-2 mt-1">
                                        <template v-for="media in review.reviewMedia" :key="media.id">
                                ...
                                <div class="flex justify-between items-center mt-3">
                                    <div class="flex items-center gap-3">
                                        <i class="fa fa-thumbs-up fa-3x"
                                            ...
                                            :style="{ color: review.isLiked ? '#74C0FC' : 'gray' }"
                                            aria-hidden="true"></i>
                                    ...
                                    <div class="flex gap-32 ">
                                        <div class="flex items-center gap-3">
                                            <button @click="toggleComment(reviewIdx)"
                                                ...
                                                💬 {{ reviewStates[reviewIdx].isAddCommentOpen ? 'Anulează' :
                                                    'Comentează'
                                                }}
                                            </button>
                                            <button @click="toggleSectionComments(reviewIdx)"
                                                class="text-indigo-500 text-base flex items-center gap-2 hover:text-indigo-700 transition">
                                                👀 {{ reviewStates[reviewIdx].isCommentSectionOpen ? 'Anulează' :
                                ...
                                <div v-if="isLoggedIn() && review.user.id === $page.props.user.id"
                                    ...
                                    <button @click="editReview(reviewIdx)"
                                        ...
                                        ✏️ {{ reviewStates[reviewIdx].editReviewForm ? 'Anulează' : 'Editează' }}
                                    </button>
                    ...
                    <GenericDeleteNotification :open="reviewStates[reviewIdx].isDeleteDialogOpen"
                        ...
                        message="Are you sure you want to delete this review?" :objectId="review.id"
