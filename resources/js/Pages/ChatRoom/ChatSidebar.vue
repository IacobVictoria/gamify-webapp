<template>
    <div
        class="w-full md:w-1/3 bg-white shadow-md rounded-lg p-5 h-full z-50 overflow-y-auto"
    >
        <button
            @click="$emit('close')"
            class="text-sm text-gray-500 hover:text-red-500 float-right mb-2"
        >
            ✖
        </button>
        <input
            ...
            >
                ...
                <div class="flex items-center gap-3">
                    <img
                        ...
                        :src="
                            conversation.friend.gender === 'Male'
                                ? '/images/male.png'
                                : '/images/female.png'
                        "
                        alt="User Avatar"
...
export default {
    ...
    methods: {
        async searchFriendConversation() {
            try {
                const response = await axios.get(
                    "/user/user_chat/searchFriendConversation",
                    {
        ...
        selectConversation(friend) {
            ...
            const conversation = this.conversations.find(
                (convo) => convo.friend.id === friend.id
            );
            if (conversation) {
    ...
    mounted() {
        ...
        window.Echo.private(`user_message.${this.currentUser.id}`).listen(
            ".MessageUnreadUpdated",
            (event) => {
                const conversation = this.conversations.find(
                    (convo) => convo.friend.id === event.friendId
                );

                if (conversation) {
                    if (
                        this.selectedConversation !== event.friendId &&
                        !conversation.hasBeenOpened
                    ) {
        ...
        window.Echo.channel("chat_status").listen(
            ".UserStatusChanged",
            (event) => {
                const conversation = this.conversations.find(
                    (convo) => convo.friend.id === event.userId
                );
                if (conversation) {
